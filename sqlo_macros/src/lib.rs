...
fn extract_table_name(input: &DeriveInput) -> syn::Result<Option<String>> {
    ...
    for attr in &input.attrs {
        if !attr.path.is_ident("sqlo") {
            continue;
        }

        let meta = attr
            .parse_meta()
            .map_err(|_| syn::Error::new_spanned(attr, "invalid #[sqlo] attribute"))?;
        ...
        for nested in list.nested {
            match nested {
                NestedMeta::Meta(Meta::NameValue(nv)) if nv.path.is_ident("table_name") => match nv
                    .lit
                {
                    Lit::Str(ref s) => {
                        if value.is_some() {
                            return Err(syn::Error::new_spanned(nv, "duplicate key `table_name`"));
                        }
                        value = Some(s.value());
                    }
                    other => {
                        return Err(syn::Error::new_spanned(
                            other,
                            "expected string literal: #[sqlo(table_name = \"items\")]",
                        ));
                    }
                },
                NestedMeta::Meta(Meta::NameValue(nv)) => {
                    return Err(syn::Error::new_spanned(nv, "unknown key in #[sqlo]"));
                }
                other => {
                    return Err(syn::Error::new_spanned(other, "expected name-value pair"));
                }
            }
        }
    }
...
fn build_join_codegen(
    ...
    if fks.is_empty() {
        ...
        to_sql.push(quote! { Self::#never(_) => unreachable!("no joins for this model") });
        kind_arms.push(quote! { Self::#never(_) => unreachable!("no joins for this model") });
    ...
    for fk in fks {
        ...
        variants.push(quote! { #var(sqlo_traits::JoinKind) });
        ...
        to_sql.push(quote! {
            Self::#var(kind) => match kind {
                sqlo_traits::JoinKind::Inner =>
                    format!(r#" INNER JOIN {} ON {} = {}"#, #right_table, #on_left, #on_right),
                sqlo_traits::JoinKind::Left  =>
...
#[proc_macro_derive(Query, attributes(sqlo, primary_key, foreign_key))]
pub fn derive_query(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);

    if !matches!(input.vis, Visibility::Public(_)) {
        return Error::new_spanned(&input.vis, "`Query` requires a `pub` struct")
            .to_compile_error()
            .into();
    }
    ...
    let data = match &input.data {
        Data::Struct(s) => s,
        _ => {
            return Error::new_spanned(&input.ident, "`Query` supports only structs")
                .to_compile_error()
                .into();
        }
    };
    ...
    for field in fields.iter() {
        ...
        for attr in &field.attrs {
            ...
            if attr.path.is_ident("foreign_key") {
                let meta = attr
                    .parse_meta()
                    .map_err(|_| Error::new_spanned(attr, "invalid #[foreign_key]"))
                    .unwrap();
                    ...
                    match nested {
                        NestedMeta::Meta(Meta::NameValue(nv)) if nv.path.is_ident("to") => {
                            match nv.lit {
                                Lit::Str(ref s) => {
                                    if to_value.is_some() {
                                        return Error::new_spanned(nv, "duplicate key `to`")
                                            .to_compile_error()
                                            .into();
                                    }
                                ...
                                other => {
                                    return Error::new_spanned(other, r#"expected "table.pk""#)
                                        .to_compile_error()
                                        .into();
                                }
                            }
                ...
                if parts.next().is_some() {
                    return Error::new(attr.span(), r#"invalid `to` — expected "table.pk""#)
                        .to_compile_error()
                        .into();
                }
        ...
        match classify_type(ty) {
            Kind::String => {
                ...
                let v_is_notnul = format_ident!("{}IsNotNull", field_name_pascal);
            ...
            Kind::Bool => {
                ...
                write_arms
                    .push(quote! { Self::#v_true  => { w.push(concat!(#col, " = TRUE"));  } });
                write_arms
                    .push(quote! { Self::#v_false => { w.push(concat!(#col, " = FALSE")); } });
            }
            ...
            Kind::Number => {
                ...
                let v_notbetween = format_ident!("{}NotBetween", field_name_pascal);
    ...
    let out = quote! {
        ...
        impl sqlo_traits::QueryContext for #struct_ident {
        ...
        impl sqlo_traits::SqlJoin for #join_ident {
            ...
            fn kind(&self) -> sqlo_traits::JoinKind {
                match self {
                    #(#join_kind_arms),*
                }
            }
        ...
        impl sqlo_traits::Filterable for #query_ident {
            type Entity = #struct_ident;

            fn write<W: sqlo_traits::SqlWrite>(&self, w: &mut W) {
                match self {
                    #(#write_arms),*
                }
            }
        ...
        impl sqlo_traits::Sortable for #sort_ident {
            type Entity = #struct_ident;
...
pub fn context(input: TokenStream) -> TokenStream {
    ...
    let out = quote! {
        ...
        pub mod #mod_name {
            ...
            impl sqlo_traits::QueryContext for Ctx {
                const TABLE: &'static str = <#base_ty as sqlo_traits::QueryContext>::TABLE;

                type Model = <#base_ty as sqlo_traits::QueryContext>::Model;
                type Query = <#base_ty as sqlo_traits::QueryContext>::Query;
                type Sort  = <#base_ty as sqlo_traits::QueryContext>::Sort;
                type Join  = <#base_ty as sqlo_traits::QueryContext>::Join;
            }

            pub type Where = <#base_ty as sqlo_traits::QueryContext>::Query;
            pub type Sort  = <#base_ty as sqlo_traits::QueryContext>::Sort;
            pub type Join  = <#base_ty as sqlo_traits::QueryContext>::Join;

            pub use sqlo_traits::JoinKind;
        }
...
#[proc_macro_derive(WebQuery, attributes(sqlo, primary_key, foreign_key))]
pub fn derive_webquery(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);

    if !matches!(input.vis, Visibility::Public(_)) {
        return Error::new_spanned(&input.ident, "`WebQuery` requires a public struct")
            .to_compile_error()
            .into();
    }
    ...
    let data = match &input.data {
        Data::Struct(s) => s,
        _ => {
            return Error::new_spanned(&input.ident, "`WebQuery` supports only structs")
                .to_compile_error()
                .into();
        }
    };
    ...
    let fields = match &data.fields {
        Fields::Named(named) => &named.named,
        not_supported => {
            return Error::new_spanned(not_supported, "`WebQuery` requires named fields")
                .to_compile_error()
                .into();
        }
    };
    ...
    for f in fields {
        ...
        sort_structs.push(quote! {
            ...
            pub struct #sort_wrap_ident {
                pub #fname_ident: ::sqlo_traits::DtoSortDir,
            }
        });
    ...
    let out = quote! {
        ...
        impl ::sqlo_traits::WebQueryModel for #struct_ident {
